...
But we’re not done yet, now that you can add and delete from your list, let’s use one last feature of the `<RadListView>`—item animations.

<hr data-action="start" />

* **a.** Open your `app/list/list.component.html` file and add the following `<ListViewLinearLayout>` component as a child of the existing `<RadListView>` component.

``` XML
<ListViewLinearLayout tkListViewLayout scrollDirection="Vertical"
  itemInsertAnimation="Fade" itemDeleteAnimation="Scale"></ListViewLinearLayout>
```

* **b.** Save your files. After your device refreshes with the changes you should see animations when you add and delete items from your list.

> **TODO:**: Gifs for iOS and Android.

<hr data-action="end" />

Item animations in the NativeScript UI `<RadListView>` work by providing [a layout tag](http://docs.telerik.com/devtools/nativescript-ui/Controls/Angular/ListView/item-layouts) and configuring `itemInsertAnimation` and `itemDeleteAnimation` attributes. Each attribute accepts four values—`"Default"`, `"Fade"`, `"Scale"`, and `"Slide"`—so feel free to play with this example a little to find an animation you like.

#### Background: Learning more about NativeScript UI

NativeScript UI is a set of professional user interface components that includes a full charting library. As such, it can be overwhelming looking over everything the components can do.

If you’re a video learner, you might want to start with [a tour of NativeScript UI on YouTube](https://www.youtube.com/watch?v=4JJVOxybR4E). If you like skimming documentation, the [NativeScript UI documentation](http://docs.telerik.com/devtools/nativescript-ui/Controls/Angular/overview) has a thorough list of the controls and the features they offer. And finally, if you like getting your hands on code right away, the [NativeScript UI Angular sample](https://github.com/telerik/nativescript-ui-samples-angular/) is a sample app that shows off every feature of NativeScript UI in action.
