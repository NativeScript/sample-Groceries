...
* **a.** Open your `app/app.component.ts` file and replaces its contents with the following code, which adds a new `<FlexboxLayout>` component.
...
* **a.** Open your app’s `app/app.component.ts` file, and replace the component’s template with the following code, which adds a new `<Image>` component.
