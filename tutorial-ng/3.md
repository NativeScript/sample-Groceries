...
* **f.** Save your `app/app.component.ts` and `app/shared/user/user.service.ts` files.
...
* **c.** Save your `app/shared/user/user.service.ts` and `app/app.component.ts` files, but don’t bother testing on your device quite yet. (You’ll get an error if you do). Let’s take a minute to discuss what’s going on in the code you just added first.
...
* **c.** After that, update the two user imports at the top of these files to the code below.
...
* **g.** Save your `app/app.component.ts`, `app/app.module.ts`, `app/app.routing.ts`, and `app/login/login.component.ts` files.

* **h.** After you’ve saved everything, ensure the login page continues to load as expected.
...
* **d.** Save your `app/list/list.component.ts`, app/list/list.component.html`, and `app/app.routing.ts` files. There’s nothing to test on your device quite yet.
...
* **e.** Save your `app/shared/user/user.service.ts` and `app/login/login.component.ts` files.

* **f.** After your device updates with the changes, type in your login credentials and tap the **Sign in** button. You should navigate to the new list screen.
